...
export function HashVerificationForm(): JSX.Element {
  ...
  const [shouldDisplayValidationResult, setDisplayValidationResult] = useState(false);
  ...
  const handleValidationResultMessageDismissed = useCallback(() => setDisplayValidationResult(false), []);

  const handleValidateButtonClicked = useCallback(() => {
    bcryptVerify({ password: textToValidate, hash: hashToValidate }).then((isMatching) => {
      setValidationMatching(isMatching);
      setDisplayValidationResult(true);
    });
  }, [textToValidate, hashToValidate]);

  useEffect(() => setDisplayValidationResult(false), [textToValidate]);
  ...
  useEffect(() => {
    setValidateButtonDisabled(textToValidate.length === 0 || hashToValidate.length === 0 || !isHashToValidateValid);
  ...
  return (
    <>
      ...
      <Input
        ...
        onChange={(_, data) => setTextToValidate(data.value)}
